...
var CaseSearchProgress = React.createClass({
  propTypes: {
    ...
    searchProgress: React.PropTypes.string,
  ...
  render: function () {
    ...
    return <div id="search-progress" {...props}>
      <ul className="nav nav-justified icon">
        ...
        {(() => {
          if (this.props.searchParams.fileDivisionCd != 'other') {
            return <li role="presentation" className={this.props.searchProgress == 'searchResults' ? 'active' : (this.props.searchProgress == 'requestDetails' ? '' : 'inactive')}>
              <div className="blue-line"></div>
              {(content =>
                this.props.caseId
                  ? <Link to={{ pathname: `/case-information/${this.props.caseId}`, state: { searchParams: this.props.searchParams } }}>{content}</Link>
                  : <a>{content}</a>
